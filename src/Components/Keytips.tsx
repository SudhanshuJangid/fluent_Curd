...
export const Keytips: React.FunctionComponent = () => {
  const checkboxRef = useKeytipRef<HTMLDivElement>({
    keytipProps: keytipMap.CheckboxKeytip,
  });
  const linkRef = useKeytipRef<HTMLAnchorElement>({
    keytipProps: keytipMap.LinkKeytip,
  });
  const toggleRef = useKeytipRef<HTMLDivElement>({
    keytipProps: keytipMap.ToggleKeytip,
  });
  const spinButtonRef = useKeytipRef<HTMLDivElement>({
    keytipProps: keytipMap.SpinButtonKeytip,
  });
  const dropdownRef = useKeytipRef<HTMLDivElement>({
    keytipProps: keytipMap.DropdownKeytip,
  });
  const comboboxRef = useKeytipRef<HTMLDivElement>({
  ...
  return (
    <>
      <Stack>
        <p>
          For Pivots, keytips will first show for each of the pivots. After
          selecting a pivot, the Keytips for its content are shown.
        </p>
        <Pivot>
          <PivotItem
            headerText="Pivot 1"
            keytipProps={keytipMap.Pivot1Keytip}
          ...
          <PivotItem
            headerText="Pivot 2"
            keytipProps={keytipMap.Pivot2Keytip}
            style={pivotItemStyle}
          >
            <Stack tokens={stackTokens}>
          ...
          <PivotItem
            headerText="Pivot 3"
            keytipProps={keytipMap.Pivot3Keytip}
            style={pivotItemStyle}
          >
            <ComboBox
              ref={comboboxRef}
